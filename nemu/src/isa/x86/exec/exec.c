...
#define make_group(name, item0, item1, item2, item3, item4, item5, item6, item7)                                                                                                                                                                         \
	static OpcodeEntry concat(opcode_table_, name)[8] = { /* 0x00 */ item0, item1, item2, item3, /* 0x04 */ item4, item5, item6, item7 };                                                                                                            \
...
make_group(gp1, EX(add), EX(or), EX(adc), EX(sbb), EX(and), EX(sub), EX(xor), EX(cmp))
	...
	/* 0xf6, 0xf7 */
	make_group(gp3, IDEX(test_I, test), EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY)

	/* 0xfe */
	make_group(gp4, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY)

	/* 0xff */
	make_group(gp5, EMPTY, EMPTY, EX(call_rm), EX(call), EMPTY, EMPTY, EX(push), EMPTY)
	...
	static OpcodeEntry opcode_table[512] = {
		...
		/* 0x7c */ IDEXW(J, jcc, 1), IDEXW(J, jcc, 1), IDEXW(J, jcc, 1), IDEXW(J, jcc, 1), /* 0x74 */ EMPTY, EMPTY, EMPTY, EMPTY,
		/* 0x80 */ IDEXW(I2E, gp1, 1), IDEX(I2E, gp1), EMPTY, IDEX(SI2E, gp1),
		/* 0x84 */ IDEXW(E2G, test, 1), IDEX(G2E, test), EMPTY, EMPTY,
